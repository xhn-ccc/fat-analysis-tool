...
def get_standard_data():
    data = {
        'fatty acid': [
            ...
            'C18:4n-3', 'C20:0', 'C20:1n-9', 'C20:3n-3 ', import streamlit as st
import pandas as pd
...
def calculate_shift_and_match(df_sample, time_col, area_col, std_df, tolerance):
    ...
    # 修改点：在这里直接过滤掉 "未知" 的数据
    # -----------------------------------------------------
    results_filtered = results[results['匹配结果'] != '未知'].copy()
    
    return results_filtered, found_c14, shift, c14_actual_time
    ...
    if df_sample is None:
        ...
        if st.button("🚀 开始校正并识别", type="primary"):
            ...
            if found_c14:
                ...
                st.warning("⚠️ 警告：在 11.972 ± 1.5 min 范围内未找到有效的 C14:0 峰。系统将使用 **原始标准时间** 进行匹配，准确率可能下降。")
            
            # === 展示结果 ===
            st.write("### 3. 详细识别表 (仅展示已知脂肪酸)")
            ...
            if final_df.empty:
                st.warning("结果为空：未匹配到任何已知脂肪酸，请检查容差设置或标准品时间表。")
            else:
                st.dataframe(
                ...
                st.download_button("📥 下载识别结果", out_csv, f"脂肪酸识别结果_{uploaded_file.name}.csv", "text/csv")
            'C20:2n-6', 'C20:4n-3', 'C20:4n-6（ARA）', 'C20:5n-3  (EPA)',
            'C22:1n-11', 'C22:5n-3(DPA)', 'C22:6n-3(DHA)'
        ],
        'std_time': [
